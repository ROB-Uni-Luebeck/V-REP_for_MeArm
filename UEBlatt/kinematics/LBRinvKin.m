...
%Berechnung der gewï¿½nschten Ellenbogenposition in BKS-Koordinaten
...
if (d(3)+d(5))-dsw < 1E-10
    ...
    if abs(b_ell)< 1E-10 % Spezialfall: Schulter und Handgelenk auf einer Hï¿½he -> Ellipse wird zur Linie
        J(3) = atan2(s3,ELBOW*((Ed(3,4)-S(3,4))/abs(Ed(3,4)-S(3,4)))*c3);
    else
        if abs((-W(1,4)/2)) > abs(b_ell) %normaler Fall: Schulter auï¿½erhalb Ellipse  %NOTE: corrected from orig: (-W(1,4)/2) < b
            x1 = -a_ell*sqrt(-b_ell^2+(-W(1,4)/2)^2)/(-W(1,4)/2); % x0=0 gekï¿½rzt
            if abs(Ed(2,4))>abs(x1) || abs(Ed(1,4))>abs(-W(1,4)/2) % Tangentenschnittpunkt ï¿½berschrittten?
                J(3) = atan2(s3,ELBOW*(b_ell/abs(b_ell))*c3);
            else
                J(3) = atan2(s3,-ELBOW*(b_ell/abs(b_ell))*c3); 
            end
        else %Schulter liegt innerhalb oder auf Ellipse -> keine Tangente mï¿½glich
            J(3) = atan2(s3,ELBOW*(b_ell/abs(b_ell))*c3);
        end
    end
end
